...
async function onChangeFileInput(e: Event) {
    ...
    try {
        ...
        await(await db.imageFile.clear().catch(error => { log('clear error', error) }).finally(() => { log('clear finally') }) // перед выбором новой папки, удалить старые файлы
        )        // console.log(166, 'indexeddb: ', await db.imageFile.toArray(), 'files: ', files);
        await(await db.imageFile.bulkAdd(files).catch(error => { log('bulkAdd error', error) }).finally(() => { log('bulkAdd finally') }) // сохранить новые файлы
